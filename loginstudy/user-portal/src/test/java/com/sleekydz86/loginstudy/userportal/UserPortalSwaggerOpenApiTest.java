...
class UserPortalSwaggerOpenApiTest extends RedisTestSupport {
	...
	void openApiDocumentIsPublic() throws Exception {
		...
		result.andExpect(status().isOk())
				.andExpect(jsonPath("$.info.title").value("LoginStudy User Portal"))
				.andExpect(jsonPath("$.paths['/']").exists())
				.andExpect(jsonPath("$.paths['/home']").exists());
	}
}

...
import com.sleekydz86.searchai.usage.adapter.out.ratelimit.InMemoryRateLimitPort;
...
class UsagePolicyServiceTest {
	...
	void setUp() {
		...
		AppProperties.Usage usage = new AppProperties.Usage(
			300, 300, 5000, 25000, 100000, 2.0, 10.0, 0.8, 0.9, 3.0, 30,
			Map.of()
		);
		when(appProperties.usage()).thenReturn(usage);
		service = new UsagePolicyService(
			store, runtimePolicyService, mock(LlmMetricsService.class), strategies, senders,
			new InMemoryRateLimitPort(), appProperties, mock(ModelPriceService.class)
		);
	}
	...
	@Test
	void freePlanCapsAstraViaDefaultPlan() {
		RuntimePolicyService runtimePolicyService = mock(RuntimePolicyService.class);
		when(runtimePolicyService.current()).thenReturn(new RuntimePolicy(
			300, 300, 5000, 25000, 100000, 2.0, 10.0, 0.8, 0.9, 3.0, 100,
			"heuristic", 0.8, "ASTRA", true, "FREE"
		));
		AppProperties appProperties = mock(AppProperties.class);
		when(appProperties.usage()).thenReturn(new AppProperties.Usage(
			300, 300, 5000, 25000, 100000, 2.0, 10.0, 0.8, 0.9, 3.0, 30,
			Map.of("FREE", new AppProperties.PlanDef(300, 5000, 25000, 50000, 5.0, 10, "SOL"))
		));
		UsagePolicyService freeService = new UsagePolicyService(
			store, runtimePolicyService, mock(LlmMetricsService.class),
			List.of(new MultiplierAnomalyStrategy(3.0)), List.of((t, b, s) -> {
			}),
			new InMemoryRateLimitPort(), appProperties, mock(ModelPriceService.class)
		);
		assertThat(freeService.authorize("user", RequestType.GENERAL, 10).maxAllowedTier().name()).isEqualTo("SOL");
	}

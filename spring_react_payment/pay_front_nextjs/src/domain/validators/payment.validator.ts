import { z } from 'zod';
import { containsSqlInjection, containsXss, validateUrl } from '@/src/lib/utils/security';
...
export const createPaymentSchema = z.object({
  orderNo: z
    ...
    .refine(sqlInjectionRefine, '주문번호에 허용되지 않은 문자가 포함되어 있습니다.')
    .refine(xssRefine, '주문번호에 허용되지 않은 문자가 포함되어 있습니다.'),
  productDesc: z
    .string()
    .min(1, '상품 설명은 필수입니다.')
    .max(255, '상품 설명은 255자 이하여야 합니다.')
    .trim()
    .refine(sqlInjectionRefine, '상품 설명에 허용되지 않은 문자가 포함되어 있습니다.')
    .refine(xssRefine, '상품 설명에 허용되지 않은 문자가 포함되어 있습니다.'),
  amount: z
    .number()
    .min(1, '결제 금액은 1원 이상이어야 합니다.')
    .max(1000000000, '결제 금액은 10억 원을 초과할 수 없습니다.'),
  amountTaxFree: z
    .number()
  ...
  retUrl: z
    ...
    .refine(sqlInjectionRefine, 'URL에 허용되지 않은 문자가 포함되어 있습니다.')
    .refine(xssRefine, 'URL에 허용되지 않은 문자가 포함되어 있습니다.'),
  retCancelUrl: z
    .string()
  ...
  cashReceiptTradeOption: z.string().max(10, '현금영수증 발급 타입은 10자 이하여야 합니다.').optional(),
  cardOptions: z.unknown().optional(),
  installment: z.string().max(10, '할부 제한 타입은 10자 이하여야 합니다.').optional(),
});
...
export const refundPaymentSchema = z.object({
  refundNo: z
    ...
    .trim()
    .refine(sqlInjectionRefine, '환불 번호에 허용되지 않은 문자가 포함되어 있습니다.')
    .refine(xssRefine, '환불 번호에 허용되지 않은 문자가 포함되어 있습니다.'),
  reason: z
    .string()
    .max(255, '환불 사유는 최대 255자까지 입력 가능합니다.')
    .refine((val) => !val || sqlInjectionRefine(val), '환불 사유에 허용되지 않은 문자가 포함되어 있습니다.')
    .refine((val) => !val || xssRefine(val), '환불 사유에 허용되지 않은 문자가 포함되어 있습니다.')
    .optional(),
